...
function addNewAnime(anime) {
    const addStatement = anidb.prepare(`
        INSERT INTO anime (mal_id, image_url, title, episodes, airing, genres, season, year, status, personal_status, personal_RATING, personal_comments)
        VALUES (@mal_id, @image_url, @title, @episodes, @airing, @genres, @season, @year, @status, @personal_status, @personal_RATING, @personal_comments)
        `)
...
// MODIFIES: None 
// EFFECTS: Returns the total number of anime that falls under "Watched"
function returnAnimeCountGroupedBySTiers() {
    const returnStatement = anidb.prepare(`
        SELECT COUNT(*) FROM anime WHERE personal_status = 'S-Tiers'`)
    return returnStatement.all();
}
